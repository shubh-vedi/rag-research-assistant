...
> Documents are loaded, semantically chunked, then each chunk is **contextually enriched** — an LLM reads the full document and prepends 2-3 sentences explaining what each chunk is about and where it comes from. This is Anthropic's Contextual Retrieval technique from 2024. The enriched chunks are embedded and stored in ChromaDB. At query time, vector search finds relevant candidates, and GPT-4o-mini generates a cited report.

**"What's Contextual Retrieval and why use it?"**
> When you chunk a document, each chunk loses its context. A chunk saying "Revenue increased 15% in Q3" is useless without knowing which company or which year. Contextual Retrieval fixes this by prepending each chunk with context generated by an LLM. Anthropic showed this reduces retrieval failures by 49%. It's simpler than hybrid search and more effective.

**"How do you evaluate RAG quality?"**
> I use RAGAS with four metrics: Faithfulness (is the answer grounded in context?), Answer Relevancy (does it address the question?), Context Precision (are retrieved chunks relevant?), and Context Recall (did we find everything needed?).
